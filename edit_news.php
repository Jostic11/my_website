<?php
	...
	$bdinf = R::findOne('news_offered', '`id` = ?', array($arr['news_id']));
?>
		...
		<div class="create">
			<form id="createnews" method="POST">
				<div id="create_news_div">
					...
					<input type="text" id="title_news" name="title_news" value="<? echo $bdinf['title']; ?>"> <br /> <br />

					<label for="text_news"> Текст новости: </label><br />
					<textarea name="text_news" id="text_news"><? echo $bdinf['text']; ?></textarea> <br /><br />
				</div>

				<div class="left">
					<br /> <input class="mbut" type="submit" name="new_news" id="new_news" value="Сохранить предложенную новость" />
					<div>
						<?php 
							if(!empty($errors)) echo '<div class="wrong">' . array_shift($errors) . '</div>' ;
							else {
								if($b == true) echo '<div id="right"> Вы успешно сохранили новость </div>' ;
							}
						?>
					</div>
				</div>
